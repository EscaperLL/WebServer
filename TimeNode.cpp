...
TimerNode::TimerNode(std::shared_ptr<HttpData> ptr_requestData,int time_out)
    :deleted_(false),SPHttpData_(ptr_requestData)
    {
        struct timeval now;
...
void TimerManager::addTimer(std::shared_ptr<HttpData> SPHttpData, int timeout) {
  spTimeNode new_node(new TimerNode(SPHttpData, timeout));
  timeNodeQueue_.push(new_node);
...
void TimerManager::handleExpiredEvent() {
  ...
  while (!timeNodeQueue_.empty()) {
    spTimeNode ptimer_now = timeNodeQueue_.top();
